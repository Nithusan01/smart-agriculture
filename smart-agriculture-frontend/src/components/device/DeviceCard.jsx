...
import { faEye, faInfoCircle, faTrashAlt, faClock, faSignal, faMobileAlt } from '@fortawesome/free-solid-svg-icons';

const DeviceCard = ({ device, isSelected, onSelect, onRemove }) => {
  const { selectedDevice } = useDeviceAuth();
  const { latest } = useDeviceRealtime(selectedDevice);
  const { plans } = useCultivationPlan();
  
  // Local modal state - each DeviceCard manages its own modal
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-sm ${
                ...
                <FontAwesomeIcon 
                  icon={isOnline ? faSignal : faMobileAlt} 
                  className={`text-lg ${isSelected || isOnline ? 'text-white' : 'text-gray-100'}`}
                />
              </div>
              <div>
      ...
      {/* Device Info Modal - Managed by DeviceCard itself */}
      {showInfoModal && (
        <DeviceInfoModal
